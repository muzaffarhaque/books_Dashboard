...
export default function Books() {
    ...
    async function getData(){
        setLoading(true)
      const res=await commonGetApi(`https://openlibrary.org/search.json?q=${searchValue?.toLowerCase()}&limit=${pageSizeLimit || API_LIMIT}&page=${currentPage}`);
      if(resOk(res.status)){
        // console.log(res)
        setLoading(false)
    ...
    useEffect(()=>{
     navigate({search:`q=${encodeURIComponent(searchValue)}&page=${currentPage}&limit=${pageSizeLimit || API_LIMIT}`})
     getData()
    ...
    const columns = [
        ...
        },
        {
          ...
          sorter: (a, b) => a.first_publish_year.localeCompare(b.first_publish_year),
          ellipsis: true,
        //   render: (_, record) => (record?.author_name?.[0])
        },
        {
            ...
            dataIndex: 'subject',
            key: "subject",
            sorter: (a, b) => a.subject?.[0].localeCompare(b.role?.[0]),
            ellipsis: true,
  ...
  return (
    <div>
     <div className="table-wrapper">
        <SubHeader title="Books" isSearch={true} searchHandler={(e)=>{setSearchValue(e)}} btnName="" btnHandler={()=>{}} />
        <CustomModal
        title="&nbsp;"
      ...
      >
         <p className="mb-0 fs-18-14 fw-normal d-flex align-items-center gap-3">
          <Image src={exclamationCircleIcon} className="w-24" alt="!" /><span> Are you
          sure you want to delete <span className='delete-title-Name'>{editData?.title}</span> ?</span>
        </p>
      </CustomModal>
        <Table
           dataSource={data}
           ...
           pagination={{
             ...
             total: totalItems,
             position: ["none", totalItems > (pageSizeLimit || API_LIMIT) ? "bottomCenter" : "none"],
             showTotal: (total, range) => `${range[0]}-${range[1]} of ${total} items`,
             onShowSizeChange:(curr,size)=>{setPageSizeLimit(size)}
           }}
           loading={loading}
